				...
				<?php
					if(isset($backlog["sprintlog"])){
						...
						foreach($backlog["sprintlog"] as $key){
							$str .= "<div id=\"pList_".$key->id."\" class=\"sbacklog_list nlClass\"><div id='no' style='font-weight:bold; display:inline; padding:0px; margin-left:5px'>NO:</div><div id='no_box' style=' display:inline; margin-left:-1px'>".$key->id."<div style='font-size:13px; display:inline'>(".$key->btitle.")</div></div><div style='width:100px; display:inline; margin-left:9px; font-weight: bold'>TITLE:</div><div class='title' style='width:100px; display:inline; padding:2px; border-bottom:1px solid; font-size:13px; margin-left:3px'>".character_limiter($key->title,3)."</div><div style='float:right'><a href =# style='font-size:8px; display:inline; font-weight:bold'>vote</a><div style='font-size:8px; display:inline;margin-left:3px'>0</div></div>";
							$str .= "<div class='bContent'><div style='font-weight:bold; margin-left:8px'>Full Title:</div><div style = 'width:270px; font-size:10 margin-top:15px border:1px solid; background:white; margin-left:8px'>".$key->title."</div><div style='font-weight:bold; margin-left:8px'>DESCRIPTION:</div><div style = 'width:268px; height:90px; font-size:10 margin-top:15px border:1px solid; background:white; margin-left:8px'><textarea style='height:90px;width:268px;' readonly>".$key->sdesc."</textarea></div><div style='display:inline; margin-left:9px; font-size:12px; font-weight:bold'>Level:</div><div style='display:inline; margin-left:6px; font-size:11px'>0</div><div style='display:inline'><a href = '#' style='display:inline; margin-left:176px; font-size:8px'>MODIFY</a></div></div></div>";
							$cnt++;
						}
						echo $str;
					}
...
<!--수정 20130607 끝-->	
		...
		<div class="ineer_product_btn">
			...
			<div class = "plus_backlog" id = "make_splint_window">
				<p>MAKE SPRINT</p><p>Phase_<?=$phase+1?></p>
